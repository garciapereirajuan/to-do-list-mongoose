...
module.exports = {
    create: (req, res, funcResolve) => {
        ...
        if (!data.title) {
            res && message(res, 404, 'El título de la categoría es requerido.')
            return
        }
        if (!data.author) {
            res && message(res, 404, 'El autor de la categoría es requerido.')
            return
        }
        ...
        Category.find({ author: data.author }, (err, categories) => {
            ...
            if (matchTitle.length !== 0) {
                res && message(res, 404, 'El nombre de la categoría ya existe.', {err})
                return
            }
            const matchColor = categories.filter(item => item.color && item.color === data.color)
            if (matchColor.length !== 0) {
                res && message(res, 404, 'El color está siendo usado por otra cateogría.', {err})
                return
            }
            category.save((err, category) => {
                if (!category) {
                    res && message(res, 500, 'Se produjo un error al guardar la categoría.')
                    return
                }
                if (!category) {
                    res && message(res, 500, 'Se produjo un error al guardar la categoría.')
                ...
                User.findOne({ _id: data.author }, (err, user) => {
                    if (err) {
                        res && message(res, 404, 'El autor no existe.', { err })
                        return
                    }
                    ...
                    User.findByIdAndUpdate(data.author, { categories }, (err, user) => {
                        if (err || !user) {
                            res && message(res, 500, 'Se produjo un error interno, intente más tarde.')
                            return
                        }
                        
                        funcResolve && funcResolve()
                        res && message(res, 200, 'Categoría creada correctamente.', { category: category})
                    })
